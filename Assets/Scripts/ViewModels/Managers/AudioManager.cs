...
namespace Assets.Scripts.ViewModels.Managers
{
    ...
    public class AudioManager : MonoBehaviour
    {
        ...
        public void Play(AudioClip clip, float? volume = null, bool? loop = null, float tweenVolumeSpeed = 0f)
        {
            ...
            if (Mathf.Approximately(tweenVolumeSpeed, 0f))
            {
        ...
        /// </summary>
        public void Stop(AudioClip clip, float tweenVolumeSpeed = 0f)
        {
            for (int i = _activeAudioSources.Count - 1; i >= 0; --i)
            {
                ...
                if (source.clip == clip)
                {
                    ...
                    {
                        source.DOKill();
                        source.DOFade(0f, tweenVolumeSpeed).OnComplete(() =>
                        {
                            source.Stop();
                            source.clip = null;
                            ReleaseAudioSource(source);
        ...
        /// </summary>
        public void StopAll(float tweenVolumeSpeed = 0f)
        {
            for (int i = _activeAudioSources.Count - 1; i >= 0; --i)
            {
                AudioSource source = _activeAudioSources[i];
